...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextPosition = (int(state[0][0] + dx),int(state[0][1] + dy))
            nextCorners = list(state[1])
            if(not self.walls[nextPosition[0]][nextPosition[1]]):
                for corner in self.corners:
                    if(corner == nextPosition and corner in nextCorners):
                        nextCorners.remove(corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for f1 in foods:
        maxFoodDist = 0
        for f2 in foods:
            foodDist = int((abs(f1[0] - f2[0]) + abs(f1[1] - f2[1]))/2) 
            if(maxFoodDist == 0 or maxFoodDist < foodDist):
                maxFoodDist = foodDist 
        accum = abs(state[0][0] - f1[0]) + abs(state[0][1] - f1[1]) + foodDist
        if(maxAccum == 0 or maxAccum < accum):
            maxAccum = accum
    return maxAccum
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        if(state in self.food.asList()):
            self.food[state[0]][state[1]] = False
            return True
